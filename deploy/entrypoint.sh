...
#
# The early return is also what keeps a restart working: root here has
# CAP_CHOWN but not CAP_DAC_READ_SEARCH, so once the server has created its
# 0700 subdirectories root cannot walk into them, and a recursive pass on
# every start would fail on the tree it handed over itself.
#
