...
| [22 - Go backend rewrite](proposals/stowcloud-22-go-backend.md) | the fourteen findings a read of the current tree turned up, what a Go port keeps and what it cannot, the jail without `fork`, and the defensive-coding standard the port is held to. [`proposals/go-port/`](proposals/go-port/README.md) holds the seventeen detail proposals under it | **Draft, and the only ones here describing code that does not exist**; with 12, before any work on the rewrite |
