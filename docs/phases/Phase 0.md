...
Build the gate. **No functional code ships in this phase.** The gate is the
deliverable, because every rule in proposal 1 is worth what enforces it and
nothing more, and a rule added after the code it governs is a refactor.
...
- **0a**: `go/go.mod`, `cmd/stowcloud` with argv dispatch and the subcommand set
  ...
  `num`, `apierr`) compiling but empty.
- **0b**: the `golangci-lint` configuration, the two in-tree `go vet` analysers
  (`vetgo` for D7's go-statement ban, `vetsecret` for D12), the Go half of
  `scripts/verify.sh`, and the CI job.
...
- The two analysers need a deliberate violation in a test fixture to prove they
  reject it. An analyser nobody has seen fail is an analyser that may not work.
- `-trimpath` and `CGO_ENABLED=0` on every invocation, including tests.

## Done when

- `bash scripts/verify.sh` runs its Go half and passes.
- `CGO_ENABLED=0 GOOS=linux go build ./...` succeeds for `amd64` and `arm64`.
- Both vet analysers reject a deliberate violation in a committed fixture.
