...
The smallest phase. `web/src/lib/api` is updated for the five REST changes and
nothing else in `web/` is touched.
...
- **12b**: `error-text.ts` entries for new keys, in every locale.
...
- **Do not restructure `web/src/lib/api`.** The five changed surfaces are five
  functions inside `http.ts`, and splitting that file inside an API migration
  makes the diff unreviewable. There is no `upload/` directory and there are no
  per-surface modules; an earlier draft of the proposal invented them.
- **`client.ts`, `error-text.ts` and `types.ts` are the three files that exist
  as the document describes.** `share.ts` exists but is the public-link page's
  client, not admin shares.
...
- **`etagWeak` is the one genuinely new field.** The conflict screen says "this
