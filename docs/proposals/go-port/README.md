...
The detail proposals under
[`../stowcloud-22-go-backend.md`](../stowcloud-22-go-backend.md). That document
is the decision and the shape; these are what a phase actually has to build.
...
|---|---|---|
| [0 - Findings](stowcloud-0-findings.md) | the fourteen defects a read of the Rust tree turned up, each with a location and the rule that closes it | **First**, and before arguing about any of them |
...
| [5 - Store and schema](stowcloud-5-store-and-schema.md) | the cache/state split, migrations, the pure-Go SQLite decision and its abort criteria, the fileid stability question | Phase 2 |
...
Contradictions found between the overview document, these documents, and the
code. Each is resolved here rather than left for a reader to trip over.
...
| C4 | The overview deleted `node.flags`' `PINNED` bit when dead properties move to `state.db`, without noting that `node.id` is `oc:fileid` on the wire (`crates/sc-compat-nc/src/props.rs:282`). Deleting the cache re-mints every id, and every sync client sees every file as new. | Not asserted. [5](stowcloud-5-store-and-schema.md) §4.5 raises it as the one open question Phase 2 must settle before it writes a schema, with the two candidate answers and what decides between them. |
| C5 | The overview said the distroless base ships a CA bundle, while `Cargo.toml` records `webpki-roots` being chosen precisely because it does not guarantee one. | Fixed in the overview: the OIDC client takes an explicit pool and refuses to start with an empty one. See [14](stowcloud-14-smb-and-oidc.md) §4.4.2. |

All five are now corrected in the overview as well, so the two documents agree.
The ledger stays because the reasoning is worth keeping: each entry records a
premise that looked right and was not, and the code the correction was checked
against.
