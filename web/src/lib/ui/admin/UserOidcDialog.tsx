...
export function UserOidcDialog({ user, onClose }: UserOidcDialogProps) {
  ...
  const actions = link?.linked && confirmUnlink ? <><Button variant="text" onClick={() => setConfirmUnlink(false)} disabled={unlink.isPending}>{t('common.cancel')}</Button><span className="sc-danger"><Button variant="filled" loading={unlink.isPending} onClick={submitUnlink}>{t('oidc.disconnect_anyway')}</Button></span></> : <><Button variant="text" onClick={onClose}>{t('common.close')}</Button>{link?.linked ? <span className="sc-danger"><Button variant="outlined" onClick={() => setConfirmUnlink(true)}>{t('oidc.disconnect')}</Button></span> : null}</>
  return (
