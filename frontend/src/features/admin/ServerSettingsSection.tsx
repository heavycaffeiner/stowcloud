...
import { useServerSettingsState, SERVER_SETTINGS_GROUPS, type ServerSettingsGroup, type ServerSettingsValues } from './server-settings-state'
...
const GROUPS = SERVER_SETTINGS_GROUPS
...
export function ServerSettingsSection() {
  ...
  const { values, baselines, hydrated, activeGroup, validationError, outcome, restartOutcome, pathPicker, secret, announcement } = state
  const setValues = (next: Values | ((current: Values) => Values)): void => patchState((current) => ({ values: typeof next === 'function' ? next(current.values) : next }))
  const setBaselines = (next: Record<Group, string | null> | ((current: Record<Group, string | null>) => Record<Group, string | null>)): void => patchState((current) => ({ baselines: typeof next === 'function' ? next(current.baselines) : next }))
  const setHydrated = (next: Record<Group, boolean> | ((current: Record<Group, boolean>) => Record<Group, boolean>)): void => patchState((current) => ({ hydrated: typeof next === 'function' ? next(current.hydrated) : next }))
  ...
  function card(id: string, title: string, subtitle: string, children: ReactNode) { return <AdminCard id={id} title={title} subtitle={subtitle} icon={<Icon name="settings" />} headingLevel="h4" bodyClassName="sc-server-settings-form">{children}</AdminCard> }
  ...
  if (settings.isError || !snapshot) return <section className="sc-admin-section"><h3>{t('server.server_settings')}</h3><p className="sc-admin-section-error" role="alert">{t('server.could_not_load_server_settings')}</p></section>
  ...
  return <>
    <section className="sc-admin-section"><h3>{t('server.server_settings')}</h3><p className="sc-admin-section-hint">{t('server.anything_settable_config_toml_can')}</p><nav className="sc-server-settings-nav" aria-label={t('admin.server_settings_navigation')}><div className="sc-server-settings-nav-items">{[['server-smb', 'admin.server_smb'], ['server-search', 'admin.server_search'], ['server-network', 'admin.server_network'], ['server-transfers', 'admin.server_transfers'], ['server-security', 'admin.server_security'], ['server-storage', 'admin.server_storage_paths']].map(([id, key]) => <button key={id} type="button" onClick={() => document.getElementById(id)?.scrollIntoView({ behavior: 'smooth', block: 'start' })}>{t(key)}</button>)}</div></nav>
      ...
      {card('server-thumbnail', t('server.thumbnail_settings'), t('server.thumbnail_enabled_description'), <>{toggle('thumbnail.enabled', t('server.thumbnail_enabled'))}<div className="sc-server-settings-path-row">{input('thumbnail.dir', t('server.thumbnail_storage_dir'))}<Button variant="outlined" onClick={() => setPathPicker({ mode: 'folder', key: 'thumbnail.dir' })}>{t('picker.browse_folder')}</Button></div>{!String(values['thumbnail.dir'] ?? '').trim() && emptyNote(snapshot, 'thumbnail.dir', t) ? <p className="sc-server-settings-empty-note">{emptyNote(snapshot, 'thumbnail.dir', t)}</p> : null}<p className="sc-admin-section-hint">{t('server.thumbnail_storage_dir_description')}</p>{saveButton('thumbnail')}</>)}
      {card('server-network', t('server.network'), t('server.applies_immediately_no_restart_needed'), <>{input('app_hosts', t('server.app_hosts_comma_separated'))}{!String(values.app_hosts ?? '').trim() && emptyNote(snapshot, 'app_hosts', t) ? <p className="sc-server-settings-empty-note">{emptyNote(snapshot, 'app_hosts', t)}</p> : null}<p className="sc-admin-section-hint">{t('server.app_hosts_hint')}</p>{input('trusted_proxies', t('server.trusted_proxies_comma_separated'))}{!String(values.trusted_proxies ?? '').trim() && emptyNote(snapshot, 'trusted_proxies', t) ? <p className="sc-server-settings-empty-note">{emptyNote(snapshot, 'trusted_proxies', t)}</p> : null}{hop ? <p className="sc-admin-section-hint">{t('server.requests_arriving_from', { address: hopAddress || t('server.unknown_address') })} {hop.peer_trusted ? t('server.peer_trusted') : t('server.peer_not_trusted')}</p> : null}{hop && !hop.peer_trusted && hop.forwarded_seen ? <div className="sc-admin-section-warning" role="alert"><p>{t('server.forwarding_headers_ignored_hint', { address: hopAddress })}</p><Button variant="text" onClick={addObserved} disabled={!hopAddress}>{t('server.add_observed_address_to_trusted', { address: hopAddress })}</Button></div> : null}{input('content_hosts', t('server.content_hosts_comma_separated'))}{input('allowed_origins', t('server.allowed_origins_cors_comma_separated'))}{input('compat_canonical_url', t('server.compat_canonical_url'))}{input('bind', t('server.bind_address'))}<p className="sc-admin-section-hint">{t('server.bind_address_hint')}</p>{saveButton('network')}</>)}
      {card('server-watch', t('server.file_watching'), t('server.what_file_watching_is_for'), <>{input('watch.hot_set_max', t('server.maximum_folders_watched_at_once'), { type: 'number' })}{input('watch.full_threshold', t('server.changes_before_a_full_rescan'), { type: 'number' })}<p className="sc-admin-section-hint">{t('settings.within_kernel_watch_limit', { limit: snapshot.fields.find((field) => field.key === 'watch.hot_set_max')?.range && 'max' in snapshot.fields.find((field) => field.key === 'watch.hot_set_max')!.range! ? (snapshot.fields.find((field) => field.key === 'watch.hot_set_max')!.range as { max: number }).max : '' })}</p>{saveButton('watch')}</>)}
      {card('server-homes', t('server.home_folders'), t('server.home_folders_hint'), <>{toggle('homes.enabled', t('server.enable_home_folders'))}<div className="sc-server-settings-path-row">{input('homes.root', t('server.homes_root_path'))}<Button variant="outlined" onClick={() => setPathPicker({ mode: 'folder', key: 'homes.root' })}>{t('picker.browse_folder')}</Button></div>{saveButton('homes')}</>)}
      ...
      {card('server-security', t('settings.single_sign_on'), t('server.single_sign_on_hint'), <>{toggle('oidc.enabled', t('settings.oidc_enable'))}{input('oidc.issuer', t('settings.oidc_issuer'))}{input('oidc.client_id', t('settings.oidc_client_id'))}{input('oidc.secret', t('common.password'), { type: 'password', placeholder: t('settings.secret_is_write_only') })}{toggle('oidc.public_client', t('settings.oidc_public_client'))}{input('oidc.scopes', t('settings.oidc_scopes'))}{input('oidc.display_name', t('settings.oidc_display_name'))}{toggle('oidc.allow_private_endpoints', t('settings.oidc_allow_private_endpoints'))}<div className="sc-server-settings-path-row">{input('oidc.ca_cert_file', t('field.oidc_ca_cert_file'))}<Button variant="outlined" onClick={() => setPathPicker({ mode: 'file', key: 'oidc.ca_cert_file' })}>{t('picker.browse_file')}</Button></div><p className="sc-admin-section-hint">{t('server.connected_accounts_cannot_use_smb')}</p>{saveButton('oidc')}{endpoints.data && (endpoints.data.redirect_uris.length || endpoints.data.post_logout_redirect_uris.length) ? <div className="sc-server-settings-endpoints"><p className="sc-admin-section-hint">{t('settings.oidc_endpoints_hint')}</p><h5 className="sc-admin-section-subhead">{t('settings.oidc_effective_redirect_uris')}</h5>{endpoints.data.redirect_uris.map((uri) => <div className="sc-server-settings-endpoint-row" key={uri}><code className="sc-server-settings-endpoint-uri">{uri}</code><Button variant="text" ariaLabel={t('common.copy_named', { name: uri })} onClick={() => void copyEndpoint(uri, t('settings.oidc_effective_redirect_uris'))}>{t('common.copy')}</Button></div>)}<h5 className="sc-admin-section-subhead">{t('settings.oidc_post_logout_redirect_uris')}</h5>{endpoints.data.post_logout_redirect_uris.map((uri) => <div className="sc-server-settings-endpoint-row" key={uri}><code className="sc-server-settings-endpoint-uri">{uri}</code><Button variant="text" ariaLabel={t('common.copy_named', { name: uri })} onClick={() => void copyEndpoint(uri, t('settings.oidc_post_logout_redirect_uris'))}>{t('common.copy')}</Button></div>)}<p className="sc-server-settings-announce" aria-live="polite">{announcement}</p></div> : null}</>)}
    </section>
