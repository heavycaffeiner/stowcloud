      ...
      {usersQuery.isPending ? <mdui-circular-progress /> : usersQuery.error ? <p className="sc-admin-section__error" role="alert">{describeApiError(usersQuery.error, t('user.could_not_load_user_list'))}</p> : (
        <VirtualList
          ...
          renderItem={(user) => {
            ...
            return <ListItem
              headline={<><span className="sc-admin-row__name">{user.display_name || user.name}</span>{user.is_admin ? <span className="sc-admin-chip">{t('common.administrator')}</span> : null}{user.disabled ? <span className="sc-admin-chip sc-admin-chip--muted">{t('user.inactive')}</span> : null}</>}
