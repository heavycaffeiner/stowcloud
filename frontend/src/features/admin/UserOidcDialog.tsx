...
export function UserOidcDialog({ user, onClose }: UserOidcDialogProps) {
  ...
  return (
    <Dialog open={!!user} title={user ? t('oidc.single_sign_for', { name: user.display_name || user.name }) : t('settings.single_sign_on')} onClose={onClose} actions={actions}>
      {query.isPending ? <ProgressCircular /> : loadError ? <p className="sc-user-oidc-error" role="alert">{loadError}</p> : link?.linked ? <><dl className="sc-user-oidc-facts"><div><dt>{t('oidc.provider')}</dt><dd>{link.issuer}</dd></div><div><dt>{t('oidc.subject_sub')}</dt><dd><code>{link.subject}</code></dd></div><div><dt>{t('oidc.linked_on')}</dt><dd>{link.linked_ns ? formatDateNs(link.linked_ns) : '-'}</dd></div><div><dt>{t('oidc.last_sign')}</dt><dd>{link.last_login_ns ? formatDateNs(link.last_login_ns) : t('oidc.never')}</dd></div></dl>{confirmUnlink ? <><p className="sc-user-oidc-warning">{t('oidc.disconnecting_here_cannot_restore_smb')}</p>{unlink.error ? <p className="sc-user-oidc-error" role="alert">{describeError(unlink.error, t('oidc.could_not_disconnect_try_again'))}</p> : null}</> : <p className="sc-user-oidc-hint">{t('oidc.disconnecting_signs_account_out_every')}</p>}</> : <>{unlink.isSuccess ? <p className="sc-user-oidc-warning">{t('oidc.disconnected_smb_access_account_will')}</p> : null}<p className="sc-user-oidc-hint">{t('oidc.no_identity_connected_user_must_link')}</p></>}
    </Dialog>
  )
}
