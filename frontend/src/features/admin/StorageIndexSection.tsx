...
export function StorageIndexSection() {
  ...
  return (
    <>
      <AdminCard id="storage" title={t('common.storage')} subtitle={t('admin.server_storage_paths')} icon={<Icon name="storage" />}>
        {storage.isPending ? (
          <ProgressCircular />
        ) : storage.error ? (
          ...
          <>
            ...
            <div className="sc-index-cost">
              {estimate.data ? (
                <>
                  <dl>
                    <div><dt>{t('storage.files_to_index')}</dt><dd>{formatNumber(estimate.data.files)}</dd></div>
                    <div><dt>{t('storage.disk_space_needed')}</dt><dd>{formatBytes(estimate.data.index_bytes)}</dd></div>
                    <div><dt>{t('storage.time_to_build')}</dt><dd>{formatDuration(estimate.data.build_secs)}</dd></div>
                  </dl>
              ...
              <div className="sc-admin-row"><Button variant="outlined" loading={estimate.isFetching} onClick={estimateCost}>{estimate.data ? t('storage.measure_again') : t('storage.measure_the_cost')}</Button></div>
              {estimate.error ? <p className="sc-admin-error">{describeApiError(estimate.error, t('storage.could_not_compute_estimate'))}</p> : null}
            </div>

            <div className="sc-admin-row"><Button variant="filled" loading={build.isPending} disabled={!nameEnabled || job.data?.state === 'running'} onClick={startBuild}>{t('storage.start_index_build')}</Button></div>
