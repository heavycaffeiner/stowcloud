...
import { recoveryCodesQuery, reissueRecoveryCodesMutation, totpDisableMutation, totpEnrollMutation, totpSetupMutation } from '../../lib/query/account'
...
export function TotpSection() {
  ...
  const [state, setState] = useComponentState<TotpState>({ enrollOpen: false, enrollPassword: '', setupSecret: '', setupUrl: '', enrollCode: '', secretCopyState: 'idle', recoveryCodes: null, recoveryAcknowledged: false, recoveryCopyState: 'idle', disableOpen: false, disablePassword: '', reissueOpen: false, reissuePassword: '' })
  const { enrollOpen, enrollPassword, setupSecret, setupUrl, enrollCode, secretCopyState, recoveryCodes, recoveryAcknowledged, recoveryCopyState, disableOpen, disablePassword, reissueOpen, reissuePassword } = state
  const update = <K extends keyof TotpState>(key: K, value: TotpState[K]): void => setState((current) => ({ ...current, [key]: value }))
  ...
  return (
    <div className="sc-totp">
      <div className="sc-totp-status"><span className={enabled ? 'sc-totp-badge sc-totp-badge--on' : 'sc-totp-badge'}>{enabled ? t('totp.on') : t('totp.off')}</span>{enabled ? <Button variant="outlined" onClick={openDisable}>{t('totp.turn_off_two_factor_authentication')}</Button> : <Button onClick={openEnroll}>{t('totp.set_up_two_factor_authentication')}</Button>}</div>
      {enabled ? <div className="sc-totp-recovery">{recovery.data ? <p className={recovery.data.remaining <= 3 ? 'sc-totp-recovery-count sc-totp-recovery-count--low' : 'sc-totp-recovery-count'}>{tp('totp.recovery_codes_left', recovery.data.remaining)}{recovery.data.remaining <= 3 ? ` ${t('totp.running_low_reissue_them_now')}` : ''}</p> : null}<Button variant="outlined" onClick={openReissue}>{t('totp.reissue_recovery_codes')}</Button></div> : null}
      <SettingsDialog open={enrollOpen} title={t('totp.set_up_two_factor_authentication')} onClose={closeEnroll} actions={<><Button variant="text" onClick={closeEnroll} disabled={setup.isPending || enroll.isPending}>{t('common.cancel')}</Button><Button onClick={setupSecret ? confirmEnroll : revealSecret} disabled={setupSecret ? enrollCode.length !== 6 : !enrollPassword} loading={setup.isPending || enroll.isPending}>{setupSecret ? t('totp.enable') : t('common.continue')}</Button></>}>
        ...
        {setupSecret ? <><p>{t('totp.add_key_below_authenticator_app')}</p><div className="sc-totp-secret-row"><input className="sc-totp-secret" readOnly value={setupSecret} aria-label={t('totp.add_key_below_authenticator_app')} /><Button variant="text" onClick={() => void copySecret()}>{secretCopyState === 'copied' ? t('common.copied') : t('common.copy')}</Button></div>{secretCopyState === 'failed' ? <p className="sc-totp-copy-feedback" role="alert">{t('totp.copy_secret_failed')}</p> : null}<p className="sc-totp-url">{setupUrl}</p><TextField label={t('totp.6_digit_code')} value={enrollCode} error={enrollErrorText} onValueChange={setEnrollCode} /></> : enrollErrorText ? <p className="sc-totp-smb-warning" role="alert">{enrollErrorText}</p> : null}
      </SettingsDialog>
      <SettingsDialog open={disableOpen} title={t('totp.turn_off_two_factor_authentication_2')} onClose={() => { if (!disable.isPending) setDisableOpen(false) }} actions={<><Button variant="text" onClick={() => setDisableOpen(false)} disabled={disable.isPending}>{t('common.cancel')}</Button><Button onClick={confirmDisable} disabled={!disablePassword} loading={disable.isPending}>{t('totp.turn_off')}</Button></>}>
        <p>{t('totp.enter_your_current_password_continue')}</p>{smbDedicated ? <p className="sc-totp-smb-warning">{t('smb.dedicated_will_be_replaced')}</p> : <p className="sc-totp-smb-warning">{t('smb.remove_reverts_to_account')}</p>}<TextField type="password" label={t('common.current_password')} value={disablePassword} error={disableError} onValueChange={setDisablePassword} />
      </SettingsDialog>
      ...
      <SettingsDialog open={!!recoveryCodes} title={t('totp.recovery_codes')} onClose={closeRecoveryCodes} dismissible={false} actions={<Button onClick={acknowledgeRecoveryCodes}>{t('totp.acknowledge_codes_saved')}</Button>}>
        <p>{t('totp.each_code_works_once_save')}</p><ul className="sc-totp-codes">{(recoveryCodes ?? []).map((code) => <li key={code}><input className="sc-totp-code" readOnly value={code} aria-label={t('totp.recovery_codes')} /></li>)}</ul><Button variant="outlined" onClick={() => void copyRecoveryCodes()}>{recoveryCopyState === 'copied' ? t('common.copied') : t('totp.copy_codes')}</Button>{recoveryCopyState === 'failed' ? <p className="sc-totp-copy-feedback" role="alert">{t('totp.copy_codes_failed')}</p> : null}
      </SettingsDialog>
    </div>
  )
}
