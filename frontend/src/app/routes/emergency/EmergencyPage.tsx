  ...
  return (
    <main className="sc-emergency">
      <div className="sc-emergency__card">
        ...
        {step === 'setup' ? <><p className="sc-emergency__hint">{t('emergency.no_administrator_yet')}</p><div className="sc-emergency__actions"><Button onClick={() => { window.location.href = '/setup' }}>{t('emergency.go_to_setup')}</Button></div></> : null}
        {step === 'credentials' || step === 'totp' ? (
          <form className="sc-emergency__form" onSubmit={flows.signIn}>
            {step === 'credentials' ? <><TextField value={username} label={t('login.username')} autoFocus autoComplete="username" onValueChange={actions.setUsername} /><TextField value={password} label={t('common.password')} type="password" autoComplete="current-password" onValueChange={actions.setPassword} /></> : <><p className="sc-emergency__hint">{t('emergency.enter_your_code')}</p><TextField value={code} label={t('login.verification_code')} autoFocus autoComplete="one-time-code" onValueChange={actions.setCode} /></>}
        ...
        {step === 'editing' ? (
          <>
            <dl className="sc-emergency__facts"><dt>{t('server.bind_address')}</dt><dd><code>{listen}</code></dd><dt>{t('server.app_hosts_comma_separated')}</dt><dd><code>{appHosts.join(', ') || t('emergency.none')}</code></dd></dl>
            <form className="sc-emergency__form" onSubmit={(event) => { event.preventDefault(); void flows.saveCurrentSection() }}>
              ...
              <textarea id="sc-emergency-doc" className="sc-emergency__doc" rows={14} spellCheck={false} disabled={sectionLoading || busy} value={documentText} onChange={(event) => actions.setDocumentText(event.target.value)} />
              <p className="sc-emergency__hint">{t('emergency.document_hint')}</p>
              <div className="sc-emergency__actions"><Button type="submit" loading={busy} disabled={sectionLoading}>{t('common.save')}</Button><Button variant="outlined" onClick={() => void flows.restart()} loading={busy} disabled={sectionLoading}>{t('emergency.restart_now')}</Button></div>
            </form>
