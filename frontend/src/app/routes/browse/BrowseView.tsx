...
export function BrowseToolbar(props: ToolbarProps) {
  ...
  return <header className={`sc-browse__toolbar${compact ? ' sc-browse__toolbar--compact' : ''}`}>
    <div className="sc-browse__folder-heading">
      ...
      {external ? <span className="sc-browse__external-badge"><Icon name="warning" size={14} />{t('common.shared_with_other_services')}</span> : null}
      {encrypted ? unlocked ? <span className="sc-browse__encrypted-badge"><Icon name="lock" size={14} />{t('browse.encrypted_badge')}</span> : <button type="button" className="sc-browse__encrypted-badge sc-browse__encrypted-badge--locked" onClick={onUnlock}><Icon name="lock" size={14} />{t('browse.encrypted_locked_badge')}</button> : null}
      {broken ? <span className="sc-browse__broken-badge"><Icon name="warning" size={14} />{t('browse.this_folder_is_unavailable')}</span> : null}
    </div>
    <div className="sc-browse__toolbar-actions">
      {!compact ? <>
        <button type="button" className={`sc-browse__filter-pill${filterType !== 'all' ? ' sc-browse__filter-pill--active' : ''}`} aria-label={t('browse.filter_type')} onClick={(event) => onFilter('type', event)}><span>{filterTypeLabel}</span><Icon name="arrow-drop-down" size={16} /></button>
        <button type="button" className={`sc-browse__filter-pill${filterDate !== 'any' ? ' sc-browse__filter-pill--active' : ''}`} aria-label={t('browse.filter_date')} onClick={(event) => onFilter('date', event)}><span>{filterDateLabel}</span><Icon name="arrow-drop-down" size={16} /></button>
        <button type="button" className="sc-browse__action-btn sc-icon-button" aria-label={t('common.refresh')} onClick={onRefresh}><Icon name="refresh" size={18} /></button>
        <button type="button" className="sc-browse__action-btn sc-icon-button" aria-label={mode === 'list' ? t('browse.grid_view') : t('browse.list_view')} onClick={onToggleView}><Icon name={mode === 'list' ? 'grid' : 'list'} size={18} /></button>
        <button type="button" className={`sc-browse__action-btn sc-icon-button${details ? ' is-active' : ''}`} aria-label={details ? t('details.hide') : t('details.show')} onClick={onToggleDetails}><Icon name="info" size={18} /></button>
        <button type="button" className="sc-browse__action-btn sc-icon-button" aria-label={sortLabel} onClick={onSort}><Icon name="sort" size={18} /></button>
      </> : <>
        {canCreate ? <button type="button" className="sc-browse__fab-btn" aria-label={t('browse.new')} onClick={onNew}><Icon name="add" size={20} /></button> : null}
        <button type="button" className="sc-browse__action-btn sc-icon-button" aria-label={sortLabel} onClick={onSort}><Icon name="sort" size={20} /></button>
        <button type="button" className="sc-browse__action-btn sc-icon-button" aria-label={t('browse.more')} onClick={onOverflow}><Icon name="more-vert" size={20} /></button>
      </>}
    </div>
  </header>
}
...
export function BrowseContent(props: BrowseContentProps) {
  ...
  return <div className="sc-browse__content">
    {treeOpen ? <FileTree currentPath={path} onNavigate={onTreeNavigate} overlay={compact} onClose={onTreeClose} /> : null}
    <div className={`sc-browse__table-wrap${dragOver ? ' sc-browse__table-wrap--dragover' : ''}${marqueeRect ? ' sc-browse__table-wrap--marquee' : ''}`} onPointerDown={onPointerDown} onContextMenu={onBlankMenu} onClick={onEmptyClick}>
      {noShares ? <div className="sc-browse__nothing"><div className="sc-browse__nothing-icon" aria-hidden="true"><Icon name="folder" size={40} /></div><h2 className="sc-browse__nothing-title">{t('browse.nothing_here')}</h2><p className="sc-browse__nothing-hint">{isAdmin ? t('browse.press_this_button_to_set_up_your_first_folder') : t('browse.ask_an_administrator_for_a_folder')}</p>{isAdmin ? <Button onClick={onAddFolder}>{t('common.add_folder')}</Button> : null}</div> : isPending ? <div className="sc-browse__loading"><mdui-circular-progress /></div> : error ? <p className="sc-browse__error" role="alert">{errorText}</p> : <div className="sc-browse__view">{mode === 'list' ? <FileTable ref={tableRef} entries={filteredEntries} total={all ? directory.total : filteredEntries.length} dirs={dirs} loading={isPending} loadingMore={isFetchingMore} requestMore={onRequestMore} perms={directory.perms} onOpen={onOpen} onContextMenu={onContextMenu} onRename={onRename} onDelete={onDelete} onSearchFocus={onSearchFocus} encrypted={encrypted} /> : <FileGrid ref={gridRef} entries={filteredEntries} total={all ? directory.total : filteredEntries.length} dirs={dirs} loading={isPending} loadingMore={isFetchingMore} requestMore={onRequestMore} perms={directory.perms} onOpen={onOpen} onContextMenu={onContextMenu} onRename={onRename} onDelete={onDelete} onSearchFocus={onSearchFocus} encrypted={encrypted} />}</div>}
...
export function BrowseDialogs(props: BrowseDialogsProps) {
  ...
  return <>
    ...
    {operation ? <section className="sc-browse__operation" role="status" aria-live="polite"><div className="sc-browse__operation-heading"><h2>{operation.kind === 'delete' ? t('common.delete') : operation.kind === 'move' ? t('common.move') : t('common.copy')}</h2><button type="button" className="sc-browse__operation-close" onClick={() => onPatch({ operation: null })}>{t('common.close')}</button></div><VirtualList items={operation.results} itemKey={(result) => result.path} estimateSize={48} itemProps={(result) => ({ className: !result.ok ? 'sc-browse__operation-error' : undefined })} renderItem={(result) => <><span>{result.destination ? `${result.path} to ${result.destination}` : result.path}</span><span>{result.ok ? result.skipped ? t('browse.items_skipped_name_taken', { count: 1 }) : t('common.done') : t('error.internal')}</span></>} />{operation.results.some((result) => result.skipped) ? <p>{t('browse.items_skipped_name_taken', { count: operation.results.filter((result) => result.skipped).length })}</p> : null}</section> : null}
