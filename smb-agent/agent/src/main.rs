...
//!
//! The server renders `smb.conf`, `smbpasswd`, `passwd` and `network.policy`
...
// The two things the server and this agent both have to agree on. Vendored at
// the cutover: the server is a Go program now, so there is no crate to share
// them through, and the agreement is kept by both sides' tests instead.
mod shared;
