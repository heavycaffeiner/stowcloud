...
struct Cli {
    ...
    #[arg(long, env = "SC_SMB_SOCKET", default_value = crate::shared::agent::DEFAULT_SOCKET)]
    socket: PathBuf,
...
pub fn main() -> std::process::ExitCode {
    ...
    while !STOP.load(Ordering::Relaxed) {
        ...
        // rewrite included.
        let wanted_up = agent.last().smbd != crate::shared::agent::SmbdAction::Stopped;
...
fn install_signal_handler() {
    ...
    // async-signal-safe.
    unsafe {
        // Through a pointer, not straight to an integer: a function item is
        // zero-sized, so the direct cast converts the type rather than taking
        // an address, and what the kernel is handed is not the handler.
