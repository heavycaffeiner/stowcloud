...
func TestTheRecordedStatusIsTheOneSent(t *testing.T) {
	...
	if e := sink.last(t); e.Status != http.StatusNotFound {
...
func TestARefusedRequestIsRecorded(t *testing.T) {
	...
	if got := send(t, app, req).status; got != http.StatusMisdirectedRequest {
		t.Fatalf("the request answered %d", got)
	}
	...
	if e := sink.last(t); e.Status != http.StatusMisdirectedRequest {
