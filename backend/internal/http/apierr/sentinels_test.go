...
func classifiedNames(t *testing.T) map[string]bool {
	...
	for _, line := range strings.Split(string(body), "\n") {
		for _, pkg := range []string{"core.", "files.", "auth.", "upload.", "uploads.", "preview.", "oidc."} {
...
func TestEveryServiceSentinelIsClassified(t *testing.T) {
	...
	for _, dir := range servicePackages() {
		...
		switch packageName {
		case "files":
			aliases = append(aliases, "core")
		case "uploads":
			aliases = append(aliases, "upload")
		}
		for _, sentinel := range declaredSentinels(t, dir) {
			total++
			mapped := slices.ContainsFunc(aliases, func(alias string) bool { return named[alias+"."+sentinel] })
			if !mapped {
				t.Errorf("%s.%s is exported and this package does not classify it, "+
					"so it would answer 500", packageName, sentinel)
			}
		}
	}
	if total == 0 {
...
func TestTheTableCoversEveryServicePackage(t *testing.T) {
	...
	for _, dir := range servicePackages() {
		...
		for name := range named {
			if slices.ContainsFunc(aliases, func(alias string) bool { return strings.HasPrefix(name, alias+".") }) {
				found = true
				break
			}
		}
		if !found {
			t.Errorf("the table classifies nothing from %s", packageName)
		}
	}
}
