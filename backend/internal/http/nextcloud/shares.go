...
func (s *Server) listShares(c *gin.Context, p Principal) (Val, bool, *Error) {
...
// shared.
func (s *Server) sharesUnderRoot(c *gin.Context, p Principal) []Val {
	ctx := c.Request.Context()
	items := make([]Val, 0)
...
func (s *Server) createShare(c *gin.Context, p Principal) (Val, bool, *Error) {
	ctx := c.Request.Context()
...
func (s *Server) getShare(c *gin.Context, p Principal, id string) (Val, bool, *Error) {
	ctx := c.Request.Context()
...
func (s *Server) updateShare(c *gin.Context, p Principal, id string) (Val, bool, *Error) {
	storeID, ok := parseLinkID(id)
	if !ok {
		return Val{}, false, NotFound("The requested share could not be found")
	}
...
func (s *Server) updateLinkShare(c *gin.Context, p Principal, id int64) (Val, bool, *Error) {
	ctx := c.Request.Context()
	owner := user(p)
...
func (s *Server) deleteShare(c *gin.Context, p Principal, id string) (Val, bool, *Error) {
	ctx := c.Request.Context()
	storeID, ok := parseLinkID(id)
	if !ok {
		return Val{}, false, NotFound("The requested share could not be found")
...
func parseSharePatchBody(c *gin.Context) sharePatchBody {
	...
	if _, ok := c.Request.Form["permissions"]; ok {
		if n, err := strconv.ParseInt(c.PostForm("permissions"), 10, 64); err == nil && n > 0 {
			req.permissions, req.hasPermissions = n, true
		}
	}
	if _, ok := c.Request.Form["password"]; ok {
		req.password, req.hasPassword = c.PostForm("password"), true
	}
	if _, ok := c.Request.Form["expireDate"]; ok {
		req.expireDate, req.hasExpireDate = c.PostForm("expireDate"), true
	}
	if _, ok := c.Request.Form["note"]; ok {
		req.note, req.hasNote = c.PostForm("note"), true
	}
	if _, ok := c.Request.Form["label"]; ok {
		req.label, req.hasLabel = c.PostForm("label"), true
	}
	if _, ok := c.Request.Form["publicUpload"]; ok {
		req.publicUpload, req.hasPublicUpload = queryBool(c.PostForm("publicUpload")), true
	}
	return req
}
...
// share routes.
func shareFormValue(c *gin.Context, key string) string {
	if strings.Contains(strings.ToLower(c.GetHeader("Content-Type")), "json") {
		var raw map[string]any
		if err := c.ShouldBindJSON(&raw); err != nil {
			return ""
...
func parseShareDate(raw string) (int64, error) {
	...
	return 0, errors.New("share expiry is outside the supported range")
}
