...
func TestACallbackWithoutAStateIsRefused(t *testing.T) {
	...
	for _, query := range []string{"", "?code=abc", "?state=abc", "?error=access_denied"} {
		resp, err := nc.Get(base + "/api/v1/auth/oidc/callback" + query)
		if err != nil {
			t.Fatalf("requesting: %v", err)
		}
		if cerr := resp.Body.Close(); cerr != nil {
		...
		if resp.StatusCode != http.StatusFound {
			t.Errorf("the callback %q answered %d, want a refusal redirect", query, resp.StatusCode)
		}
		if location := resp.Header.Get("Location"); location == "" {
			t.Errorf("the refused callback %q carried no error redirect", query)
		}
	}
}
...
func TestAnInventedStateCompletesNothing(t *testing.T) {
	...
	if resp.StatusCode != http.StatusFound {
		t.Fatalf("an invented state answered %d, want a refusal redirect", resp.StatusCode)
	}
	if location := resp.Header.Get("Location"); location != "/login?oidc_error=oidc.bad_state" {
		t.Fatalf("an invented state redirected to %q", location)
	}
...
func TestAConfiguredProviderBuildsAClient(t *testing.T) {
	...
	reopened, err := app.Open(ctx, app.Options{DataDir: dir, PasswordParams: fastPasswordParams()})
	if err != nil {
