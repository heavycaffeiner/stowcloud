...
type Options struct {
	...
	// InstanceLockAcquiredExternally says the caller already owns the data-directory
	// instance lock. The engine then neither takes nor releases that lock: one
...
func Open(ctx context.Context, opt Options) (*Engine, error) {
	...
	e.searchController = controller.New(controller.Options{
	...
	if uerr != nil {
		...
		if rerr := up.RecoverCache(ctx); rerr != nil {
			if cerr := up.Close(); cerr != nil {
				rerr = errors.Join(rerr, cerr)
			}
			return fail(fmt.Errorf("recovering upload cache: %w", rerr))
		}
		e.Upload = up
	}
...
func (e *Engine) drainJobs() {
	if e.searchController == nil {
		return
	}
	ctx, cancel := context.WithTimeout(context.Background(), jobDrainTimeout)
	defer cancel()
	if derr := e.searchController.DrainJobs(ctx); derr != nil {
...
func (e *Engine) Close() (err error) {
	// The engine releases only a lock it acquired. External ownership remains
	// with the caller, so Close must leave that lock untouched.
	defer func() {
		if e.lock != nil {
	...
	if e.searchController != nil {
		e.searchController.StopUpdater()
	}
	...
	if previewSvc != nil {
		if perr := previewSvc.Close(); perr != nil {
			errs = append(errs, fmt.Errorf("closing previews: %w", perr))
		}
	}
	if e.Upload != nil {
		if uerr := e.Upload.Close(); uerr != nil {
			errs = append(errs, fmt.Errorf("closing uploads: %w", uerr))
		}
		e.Upload = nil
	}
...
// previewLease obtains a request-stable service reference. The settings lock
// covers selection; the service lease covers the replacement close that follows.
func (e *Engine) previewLease() (*preview.Lease, bool) {
	e.thumbnailMu.RLock()
	defer e.thumbnailMu.RUnlock()
	if e.Preview == nil || !e.thumbnailOn {
		return nil, false
	}
	lease, ok := e.Preview.Acquire()
	return lease, ok
}
