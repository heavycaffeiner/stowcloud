...
func newFlow(t *testing.T, f fixture, now func() int64) (*auth.LoginFlow, *mintingSource) {
	t.Helper()
	src := &mintingSource{}
	flow, err := auth.NewLoginFlow(f.store, src, now)
	if err != nil {
...
// A begun flow returns two distinct secrets, and beginning again returns a
// different pair: nothing is derived from a source that would let a second
// flow calculate a token the first one still holds.
func TestABegunFlowReturnsTwoDistinctTokens(t *testing.T) {
	t.Parallel()
	f := newFixture(t)
	...
	if tokens.PollToken == "" || tokens.LoginToken == "" {
		t.Error("a begun flow carries an empty token")
	}
	if tokens.PollToken == tokens.LoginToken {
		t.Error("the two tokens are the same value")
	}

	again, err := flow.Begin(context.Background(), flowOrigin)
	if err != nil {
		t.Fatalf("beginning a second flow: %v", err)
	}
	if again.PollToken == tokens.PollToken || again.LoginToken == tokens.LoginToken {
		t.Error("a second flow reused a token from the first")
	}
}
...
func TestAnUnapprovedFlowIsPending(t *testing.T) {
	...
	if _, err := flow.Poll(ctx, tokens.PollToken, flowOrigin); !errors.Is(err, auth.ErrFlowPending) {
...
func TestASecondPollFindsNothing(t *testing.T) {
	...
	if _, err := flow.Poll(ctx, tokens.PollToken, flowOrigin); !errors.Is(err, auth.ErrFlowUnknown) {
...
func TestTheLoginTokenCannotPoll(t *testing.T) {
	...
	if _, err := flow.Poll(ctx, tokens.LoginToken, flowOrigin); !errors.Is(err, auth.ErrFlowUnknown) {
...
func TestASecondApprovalIsRefused(t *testing.T) {
	...
	if err := flow.Approve(ctx, tokens.LoginToken, 8, "mallory"); !errors.Is(err, auth.ErrFlowApproved) {
...
func TestAnExpiredFlowIsUnknown(t *testing.T) {
	...
	nowNs += int64(auth.FlowTTL) + 1

	if _, err := flow.Poll(ctx, tokens.PollToken, flowOrigin); !errors.Is(err, auth.ErrFlowUnknown) {
		t.Errorf("polling an expired flow answered %v, want unknown", err)
	}
	if err := flow.Approve(ctx, tokens.LoginToken, 7, "alice"); !errors.Is(err, auth.ErrFlowUnknown) {
...
func TestPollingTooSoonIsRefusedAndLaterAccepted(t *testing.T) {
	...
	if _, err := flow.Poll(ctx, tokens.PollToken, flowOrigin); !errors.Is(err, auth.ErrFlowRateLimited) {
		t.Errorf("an immediate second poll answered %v, want rate limited", err)
	}

	// Past the interval.
	nowNs += int64(auth.FlowPollInterval) + 1
	if _, err := flow.Poll(ctx, tokens.PollToken, flowOrigin); !errors.Is(err, auth.ErrFlowPending) {
...
func TestAnApprovedFlowDeliversPastTheLimit(t *testing.T) {
	...
	if !errors.Is(err, auth.ErrFlowPending) {
		t.Fatalf("the first poll answered %v, want pending", err)
	}
	if _, perr := flow.Poll(ctx, tokens.PollToken, flowOrigin); !errors.Is(perr, auth.ErrFlowRateLimited) {
...
func TestAnUnknownTokenIsUnknown(t *testing.T) {
	...
	if _, err := flow.Poll(ctx, "never-minted", flowOrigin); !errors.Is(err, auth.ErrFlowUnknown) {
...
func TestTheSweepRemovesExpiredFlows(t *testing.T) {
	...
	if _, err := flow.Poll(ctx, old.PollToken, flowOrigin); !errors.Is(err, auth.ErrFlowUnknown) {
		t.Errorf("a swept flow answered %v, want unknown", err)
	}
	if _, err := flow.Poll(ctx, fresh.PollToken, flowOrigin); !errors.Is(err, auth.ErrFlowPending) {
...
func TestTheFlowNeedsACredentialSource(t *testing.T) {
	t.Parallel()
	f := newFixture(t)
	if _, err := auth.NewLoginFlow(f.store, nil, nil); err == nil {
...
func TestAStoreSentinelArrivesMapped(t *testing.T) {
	...
	if _, err := flow.Poll(ctx, "absent", flowOrigin); !errors.Is(err, auth.ErrFlowUnknown) {
