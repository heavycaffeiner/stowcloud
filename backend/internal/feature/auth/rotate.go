...
func (s *Service) RotateMasterKey(ctx context.Context) (RotationReport, error) {
	...
	res, perr := extended.persist()
	if werr := wrapPublication("persisting the new master key", res, perr); werr != nil {
		return rep, werr
	}
	rep.NewVersion = next
	...
	res, perr = compacted.persist()
	if werr := wrapPublication("compacting the master key ring", res, perr); werr != nil {
		return rep, werr
	}
