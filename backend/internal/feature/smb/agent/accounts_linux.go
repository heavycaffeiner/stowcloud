...
// PublicationError preserves whether a durable write changed the destination
// when the operation also returned an error. Callers must not report an
// uncertain publication as though the old file remained visible.
...
func WritePasswd(path, body string) error {
	...
	res, err := fsatomic.ReplaceFileDurable(path, passwdMode, func(f *os.File) error {
		_, werr := f.WriteString(body)
		return werr
	})
	if err != nil {
		return &PublicationError{Operation: "replacing the account file", Outcome: res.Outcome, Err: err}
	}
	return nil
}
