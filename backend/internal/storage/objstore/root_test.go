...
func (b *fakeBucket) handler(bucket string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if auth == "" || !strings.HasPrefix(auth, "AWS4-HMAC-SHA256") {
			http.Error(w, "missing or malformed Authorization", http.StatusForbidden)
			return
		}
		bucketPrefix := "/" + bucket
...
func (b *fakeBucket) handleCopy(w http.ResponseWriter, r *http.Request, destKey string) {
	...
	if b.requireSignedCopySource {
		auth := r.Header.Get("Authorization")
		if !strings.Contains(auth, "SignedHeaders=") || !strings.Contains(auth, "x-amz-copy-source") {
	...
	if b.copyFailAt > 0 && copyCall >= b.copyFailAt {
