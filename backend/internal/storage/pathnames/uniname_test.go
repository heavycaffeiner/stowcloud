...
func TestDecomposedHangulComposes(t *testing.T) {
	...
	if got := pathnames.Normalize(jamo); got != syllable {
...
func TestALegacyPathDecodesFromItsWholeSample(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			got := strings.Join(pathnames.Components(strings.Split(raw, "/")), "/")
			if got != c.path {
...
func TestIllFormedBytesBecomeWellFormed(t *testing.T) {
	...
	// the file again.
	got := pathnames.Normalize("bad\xff\xfename.txt")
	if !utf8.ValidString(got) {
...
func TestTheCallerFallbackCarriesTheSingleByteCase(t *testing.T) {
	...
	cs := pathnames.Charset([]byte(raw), charmap.CodePage437)
	if got := pathnames.Decode([]byte(raw), cs); got != "M\u00fcnchen.txt" {
...
func TestOneSetOfBytesAlwaysDecodesToOneName(t *testing.T) {
	...
	first := strings.Join(pathnames.Components(strings.Split(raw, "/")), "/")
	for i := range 400 {
		got := strings.Join(pathnames.Components(strings.Split(raw, "/")), "/")
		if got != first {
...
func TestASingleByteGuessNeverBeatsTheFallback(t *testing.T) {
	...
	if got := pathnames.Charset(raw, nil); got != nil {
		t.Errorf("Charset returned %T for bytes only a single-byte page could explain", got)
	}
	back, _, err := transform.Bytes(charmap.ISO8859_1.NewEncoder(), []byte(pathnames.Normalize(string(raw))))
	if err != nil {
...
func TestJointDetectionBeatsPerNameDetection(t *testing.T) {
	...
	for i, b := range raws {
		if got := pathnames.Decode(b, cs); got != names[i] {
...
func TestNormalizeIsIdentityForASCII(t *testing.T) {
	for _, s := range []string{"", "plain.txt", "a/b/c.txt", "CON", "space in name.txt"} {
		if got := pathnames.Normalize(s); got != s {
