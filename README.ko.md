...
<http://127.0.0.1:8080/setup>을 열고 그 토큰을 붙여 넣은 다음, 아이디와 비밀번호를
정합니다.
...
> **바깥에서 접근하게 만들기 전에** HTTPS를 처리하는 리버스 프록시를 앞에 두고,
...
> compose 파일이 프록시용 포트를 루프백에만 여는 이유가 이것입니다. 물려받는
> 기본값이 아니라, 직접 내리는 결정이어야 합니다.

**내부망에서는** `https://<서버 주소>:8443`으로 접속하세요. compose 파일이 이
용도로 두 번째 리스너를 켜 둡니다. 서버가 스스로 만든 인증서라서 브라우저가 한
번 확인을 요구합니다. 내부망에는 공인 도메인이 없으니 그 경고가 대가입니다.
`http://` 포트는 대안이 되지 못합니다. 세션 쿠키가 보안 origin에서만 유지되기
때문에, 평문으로 로그인하면 아무 말 없이 로그인이 풀립니다.
...
- **리눅스** 커널 5.6 이상. 5.13 이상이면 이미지 미리보기에 쓰는 샌드박스까지
  동작합니다.
- **Docker** 20.10.10 이상. 그 이전 버전은 이 프로그램이 쓰는 시스템 콜을 막아서
  보안 수준을 낮춰 동작하게 만듭니다.
