...
폴더도 첫 페이지가 아니라 전부로 답합니다. 파일만, 폴더만, 또는 확장자로
좁힐 수 있습니다. 찾을 대상과 이미지, 문서 같은 종류 묶음은 필터 메뉴에
있고, 그 옆 입력창에는 확장자를 직접 적으면 됩니다. 파일 이름만 찾으며, 파일 내용 검색은 따로 켜는 기능입니다.

데스크톱에서는 검색 버튼이 보고 있던 폴더 위로 시트를 내려 주므로, 닫으면
있던 자리로 돌아옵니다. Ctrl+K, 맥에서는 Cmd+K로 어느 화면에서나 같은 시트를
엽니다. 휴대폰에서는 검색 화면으로 이동합니다.

![파일 브라우저 위에 열린 검색 시트. 2026 검색어, 필터 메뉴 버튼과 확장자 입력창, 두 공유에서 나온 결과가 폴더와 크기, 날짜와 함께 보인다](docs/screenshots/search.png)
