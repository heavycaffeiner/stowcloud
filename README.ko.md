...
> **바깥에서 접근하게 만들기 전에** 정식 인증서를 가진 리버스 프록시를 앞에 두세요.
> 인터넷에 직접 노출되지 않은 프록시는 기본적으로 신뢰하므로, 그 프록시가 전달한
> 주소가 로그인 횟수 제한과 감사 로그에 기록됩니다. 이 판단을 기본값에 맡기지 않고
> 직접 정하려면 관리자 설정 화면의 신뢰하는 프록시 범위에 주소를 적으세요. 목록을
> 한 번 지정하면 그 목록이 전부이며, 목록에 없는 프록시는 신뢰하지 않습니다.
>
...
| **검색** | 이름으로, 그 사람이 볼 수 있는 모든 폴더에서 한 번에 찾습니다. 파일만, 폴더만, 또는 지정한 확장자만으로 좁힐 수 있습니다. 찾은 것은 전부 보여 줍니다. 더 불러올 다음 페이지도, 결과 개수 제한도 없습니다. |
...
| **동기화 앱** | 기존 Nextcloud 데스크톱, 모바일 클라이언트가 수정 없이 로그인해서 동기화합니다.[^tm] 업로드 대상은 계정 최상위가 아니라 공유 안쪽으로 지정하세요. 이 서버의 최상위는 폴더가 아니라 그 계정이 받은 공유 목록이라, 그곳을 향한 업로드는 내려앉을 자리가 없습니다.[^root] |
...
필요가 없습니다. 끝까지 훑으면서 찾는 대로 흘려보내므로, 파일이 십만 개인
폴더도 첫 페이지가 아니라 전부로 답합니다. 전체, 파일, 폴더 버튼이 검색어
옆에 있어 한 번의 클릭으로 대상을 좁힙니다. 이미지, 문서 같은 종류 묶음은
파일 종류 메뉴에서 고르며, 고른 묶음은 칩으로 남아 다시 눌러 뗄 수 있습니다.
그 옆 입력창에는 확장자를 직접 적으면 됩니다. 파일 이름만 찾으며, 파일 내용
검색은 따로 켜는 기능입니다.

데스크톱에서는 검색 버튼이 보고 있던 폴더 위로 시트를 내려 주므로, 닫으면
...
샌드박스가 막을 경로는 애초에 고를 수 없습니다. 직접 입력도 그대로 됩니다.

공유 폴더가 보이는 순서는 계정마다 따로입니다. 사이드바에서 순서를 바꾸면 그
계정으로 다시 로그인해도 그대로입니다.
...
SMB는 기본적으로 꾫져 있습니다. 사이드카는 스택과 함께 띄워지고 그상 대기합니다.
스위치는 관리자 화면에 있고, 거기서 켜면 셸 없이도 데몬이 시작됩니다.

사이드카는 호스트의 네트워크 스택을 공유하므로 445 포트가 비어 있어야 합니다.
이미 Samba가 돌고 있으면 먼저 멈추세요.
...
[^root]: 모바일 앱에서는 자동 업로드 폴더 설정, 그리고 직접 올릴 때 고르는
    폴더가 이에 해당합니다. 최상위 기본값 대신 `/사진/카메라`처럼 공유 안쪽
    경로를 고르세요. 그 폴더는 앱이 만들어 주므로 미리 있을 필요는 없고, 그
    위의 공유만 있으면 됩니다. 데스크톱 클라이언트도 공유 하나를 동기화
    대상으로 잡으면 됩니다. 계정 최상위를 향한 업로드는 404로 답합니다. 그
    최상위는 무언가를 쓸 수 있는 디렉터리가 아니라 공유 목록이기 때문입니다.
