...
// observes one write's floor beside another's default is no different from two
// sessions created a moment apart.
type Settings struct {
	minBytes     atomic.Uint64
	defaultBytes atomic.Uint64

	// override records whether a row exists at all, and it is kept beside the
	// values rather than derived from them. Falling back collapses the two:
	// once it has run, "an admin stored this" and "it fell back to the config
	// file" are the same pair of integers, and the settings screen has to
	// report which.
	override atomic.Bool
}

// loadSettings seeds from the config file and lets a persisted admin override
// win, so a restart does not lose an admin's write.
//
// Precedence, in order: the persisted row, then the config file's values, then
// the compiled-in defaults. The hard floor beats all three.
func loadSettings(ctx context.Context, st *state.DB, cfgMin, cfgDefault uint64) (*Settings, error) {
	...
	if minErr != nil || defErr != nil {
		...
		// the config file's values are a working fallback and the override is
		// reported as absent, which is what it now effectively is.
		return s, nil
	}
...
// it means this server is not the one rejecting.
...
// Overridden reports whether an admin has ever written these, as opposed to
// them coming from the config file. It is the only thing that can answer that
// for the settings screen.
func (s *Settings) Overridden() bool { return s.override.Load() }

// SetChunkSettings is the admin write path: validate, persist, then move the
// live values, in that order.
//
// The order is load-bearing. A validation failure or a failed disk write never
// lets the in-memory pair drift from what is on disk, which is what a restart
// would then read back.
//
// It does not touch a session already in flight. Both numbers are snapshotted
// once at creation and never re-read, so an admin raising the floor cannot
// retroactively refuse a chunk that was legal when the session started.
func (e *Engine) SetChunkSettings(ctx context.Context, minBytes, defaultBytes uint64) error {
	if minBytes < limits.UploadChunkFloor {
		return fmt.Errorf("%w: the chunk minimum is at least %d bytes",
			ErrBadRequest, limits.UploadChunkFloor)
	}
	if defaultBytes < minBytes {
		return fmt.Errorf("%w: the chunk default is at least the minimum of %d bytes",
			ErrBadRequest, minBytes)
	}
	storedMin, minErr := num.Narrow[int64](minBytes)
	storedDefault, defErr := num.Narrow[int64](defaultBytes)
	if minErr != nil || defErr != nil {
		return fmt.Errorf("%w: a chunk setting does not fit", ErrBadRequest)
	}
	if err := e.state.WriteChunkSettings(ctx, storedMin, storedDefault); err != nil {
		return err
	}
	e.settings.store(minBytes, defaultBytes)
