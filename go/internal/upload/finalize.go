...
// that one fact. A failure before it leaves the session resumable and the part
// file on disk. After it the upload is complete even if invalidation or
// session cleanup has to be retried, because a filesystem commit cannot be
...
// finalize is the shared core both the offset-addressed and the name-ordered
// paths converge on. The caller holds the row lock.
func (e *Engine) finalize(ctx context.Context, r core.Resolved, id SessionID) (core.Entry, error) {
	row, err := e.load(ctx, id)
	if err != nil {
		return core.Entry{}, err
	}
	...
	// to. A resolution naming somewhere else is a different file, and honouring
	// it would publish through a permission check that looked at another path.
	if !dest.Equal(r.Path()) {
		return core.Entry{}, fmt.Errorf("%w: this session publishes to %s", ErrBadRequest, row.sess.Dest)
	}

	// 1. The set has to be complete, and a refusal names what is missing so
	//    the client resends the holes rather than the file.
	total, declared := row.totalLen()
	if !declared {
		return core.Entry{}, fmt.Errorf("%w: this session never declared a length", ErrBadRequest)
	}
	if !row.set.IsComplete(total) {
		return core.Entry{}, &IncompleteError{Missing: row.set.Missing(total)}
	}

	part, err := e.partPathOf(row)
	if err != nil {
		return core.Entry{}, err
	}
	root := r.Root()

	// 2. Whole-file verification, read back through the same descriptor that
	//    took the chunk writes. That handle is the one place in the tree
	//    holding IntentReadWrite and this is the reason the intent exists.
	if v := row.verify(); v != nil {
		...
		if verr := VerifyWholeFile(f, *v, total); verr != nil {
			// The session stays resumable and the part file stays on disk.
			// The client's declared digest does not match what landed, and it
			// is the client that knows whether to resend a range or start
			// again; discarding its bytes here decides that for it.
			return core.Entry{}, verr
		}
	}

	// 3. Publish. Everything from here is the vfs helper's sequence: mode and
	//    ownership restored from whatever is being replaced, fdatasync,
	//    renameat2, then the parent directory synced.
	entry, err := e.publish(ctx, r, row, part, total)
	if err != nil {
		return core.Entry{}, err
	}
	...
	if derr := e.state.DeleteUploadSession(ctx, id.Bytes()); derr != nil {
		e.log.Warn("an upload published but its session row survived; the sweep will collect it",
			slog.String("session", id.String()), slog.Any("error", derr))
	}
	return entry, nil
}
...
//
// It goes through the core's own create path rather than renaming here, so the
// precondition, the mode and ownership transplant, the cache invalidation and
// the journal row are the same ones every other write in the product gets.
func (e *Engine) publish(
	ctx context.Context, r core.Resolved, row *row, part vfs.SafePath, total uint64,
) (core.Entry, error) {
	root := r.Root()

	// The precondition is checked against what is at the destination now, not
	// against what was there when the session opened.
	if err := e.checkIfMatch(root, r.Path(), row.sess.IfMatch); err != nil {
		return core.Entry{}, err
	}

	// The descriptor goes before the rename, so nothing depends on the
	// semantics of renaming a file that is still open.
	if err := e.syncAndClose(r.Root(), row, part); err != nil {
		return core.Entry{}, err
	}

	if row.sess.MtimeNs != nil {
		if err := root.SetTimes(part, *row.sess.MtimeNs); err != nil {
			...
			e.log.Warn("could not apply the client's modification time to an upload",
				slog.String("dest", row.sess.Dest), slog.Any("error", err))
		}
	}

	entry, err := e.core.PublishPart(ctx, r, part, total)
	if err != nil {
		return core.Entry{}, err
	}
	return entry, nil
...
//
// fdatasync here guarantees the bytes; the directory sync the publish does
// afterwards is what guarantees anyone can find them. A power cut between the
// two leaves a complete upload under a name nothing looks for.
func (e *Engine) syncAndClose(root *vfs.ShareRoot, row *row, part vfs.SafePath) error {
	id, err := row.id()
	if err != nil {
		return err
	}
...
// checkIfMatch applies the client's validator to the destination as it stands.
//
// A supplied validator can never pass, and that is the file-ETag rule rather
...
func (e *Engine) checkIfMatch(root *vfs.ShareRoot, dest vfs.SafePath, ifMatch string) error {
	...
	if errors.Is(err, vfs.ErrNotFound) {
		return fmt.Errorf("%w: nothing is at the destination", ErrPrecondition)
	}
	if err != nil {
		return mapVFSErr(err)
	}
	current, _ := core.FileETag(st)
	return fmt.Errorf("%w: the destination's current token is %s", ErrPrecondition, current)
}

// Assemble is the name-ordered path's finalize: it merges whatever chunks are
// still spooled, in ascending name order, then publishes through the same core
...
// the session, and the layer that read the header is the one that can name it.
func (e *Engine) Assemble(
	...
	if dir, derr := e.spoolDirOf(row); derr == nil {
		if rerr := r.Root().Rmdir(dir); rerr != nil && !errors.Is(rerr, vfs.ErrNotFound) {
			e.log.Warn("an upload spool directory survived assembly; the sweep will collect it",
				slog.String("session", id.String()), slog.Any("error", rerr))
		}
	}
	return entry, nil
}
