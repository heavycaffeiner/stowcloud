...
func decodeBody(data []byte, f Format) (image.Image, error) {
	...
	case FormatWebP:
		...
		// than having its first frame produced, which is the one recorded
		// reduction against the previous build.
...
func Thumbnail(src image.Image, p Preset, lim DecodeLimits) (image.Image, error) {
	...
	scale := min64f(float64(maxW)/float64(w), float64(maxH)/float64(h))
	if scale >= 1 {
		// Already inside the box.
		return src, nil
	}
	outW := max1(int(float64(w) * scale))
	outH := max1(int(float64(h) * scale))

	// Both are at most the preset's own bound, which is a compiled-in
	// constant, so the widening cannot lose anything.
	outPx, perr := num.Narrow[uint64](int64(outW) * int64(outH))
	if perr != nil {
		return nil, fmt.Errorf("%w: an output of %dx%d", ErrTooLarge, outW, outH)
	}
	if px := outPx; lim.MaxOutputPixels > 0 && px > lim.MaxOutputPixels {
		return nil, fmt.Errorf("%w: an output of %d pixels exceeds the limit of %d",
			ErrTooLarge, px, lim.MaxOutputPixels)
	}

	dst := image.NewRGBA(image.Rect(0, 0, outW, outH))
	// A box filter over nearest-neighbour sampling: this is a thumbnail, and
	// the scaler is not where the interesting failures are.
	scaleInto(dst, src)
	return dst, nil
}

// scaleInto resamples src into dst by area averaging, which is what keeps a
// downscaled photo from aliasing into noise.
...
//
// PNG rather than the previous build's lossless WebP, because there is no WebP
// encoder in the standard library or in x/image. It is lossless, keeps alpha
// and writes no metadata of its own, which is what makes the EXIF strip a
// matter of never carrying anything across rather than of removing it.
