...
// turns each into a different answer to the client: unknown is a 404, a second
// approval is a conflict, and a poll that is too soon is a 429.
var (
	// ErrLoginFlowUnknown is a digest no live flow has.
	ErrLoginFlowUnknown = errors.New("state: no such login flow")
	// ErrLoginFlowApproved refuses a second approval of one flow.
	ErrLoginFlowApproved = errors.New("state: the login flow is already approved")
	// ErrLoginFlowTooSoon is a poll inside the interval the last one started.
	ErrLoginFlowTooSoon = errors.New("state: polling too fast")
)
...
func (d *DB) loginFlowBy(ctx context.Context, query string, digest []byte) (LoginFlowRow, error) {
	...
	err := d.SQL().QueryRowContext(ctx, query, digest).Scan(
...
// replayed the link.
func (d *DB) ApproveLoginFlow(ctx context.Context, loginDigest []byte, user int64, login string, nowNs int64) error {
	return d.Write(ctx, func(tx *sql.Tx) error {
		// The guard is in the statement rather than a read followed by a
		// write: two approvals arriving together would both pass a check made
		// before either wrote.
...
func (d *DB) TouchLoginFlowPoll(ctx context.Context, pollDigest []byte, nowNs, minIntervalNs int64) error {
	return d.Write(ctx, func(tx *sql.Tx) error {
		res, err := tx.ExecContext(ctx, sqlTouchLoginFlowPoll, nowNs, pollDigest, nowNs-minIntervalNs)
		if err != nil {
...
// SweepLoginFlows removes what has expired, and reports how many.
//
// An abandoned flow is the normal case: a client that begins one and never
// opens the browser leaves a row nothing will ever collect.
func (d *DB) SweepLoginFlows(ctx context.Context, olderThanNs int64) (int, error) {
	var n int
