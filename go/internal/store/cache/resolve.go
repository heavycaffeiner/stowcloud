...
// ErrNoNode is an id this cache holds no row for: never allocated, or reaped.
// It is not "no such file": the filesystem is the source of truth and this
// database is allowed not to know.
var ErrNoNode = errors.New("no such node")

// maxResolveHops bounds the parent-chain walk. A cyclic chain should not
// happen, and "should not happen" is not a proof, so a corrupt one fails
...
// UPDATE because of it, instead of a write for every row underneath.
//
// The walk is not a snapshot. A rename landing part way through yields the
// path the tree had at some point during it, which every caller of this cache
// has to tolerate anyway: the filesystem is the truth and this is a hint.
//
// A component this server would not accept, a control-file prefix among them,
// is an error rather than a repaired string.
func (d *DB) Resolve(ctx context.Context, id FileID) (vfs.ShareID, vfs.SharePath, error) {
	...
	if err != nil {
		return 0, vfs.SharePath{}, fmt.Errorf("node %d resolves to a path this server would refuse: %w", id, err)
	}
