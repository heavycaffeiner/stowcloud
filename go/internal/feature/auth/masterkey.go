...
// key never becomes readable to a neighbour of the data directory. The
// result is returned even when the operation reports an error: a directory
// sync failure means the destination may already have changed.
func (r *KeyRing) persist() (fsatomic.Result, error) {
	res := fsatomic.Result{Outcome: fsatomic.NotPublished}
	if r.filePath == "" {
		return res, errors.New("cannot persist a key ring with no file")
	}
	b, err := r.marshal()
	if err != nil {
		return res, err
...
// publicationError preserves how far a durable publication got. Callers of
// authoritative key-ring writes must not report an uncertain publication as
// though the destination was unchanged.
...
func (e *publicationError) Error() string {
	if e.err == nil {
		return fmt.Sprintf("%s: publication %s", e.operation, e.outcome)
	}
	return fmt.Sprintf("%s: publication %s: %v", e.operation, e.outcome, e.err)
}

func (e *publicationError) Unwrap() error { return e.err }

func wrapPublication(operation string, res fsatomic.Result, err error) error {
	if err == nil && res.Outcome == fsatomic.Published {
		return nil
	}
...
func (s *Service) OpenMasterKey(ctx context.Context) (*KeyRing, error) {
	...
	if !found {
		...
		ring.filePath = path
		res, perr := ring.persist()
		if werr := wrapPublication("generating the master key", res, perr); werr != nil {
			return nil, werr
		}
