...
// clients.
//
//	A client posts to begin and receives a poll token and a login token. The
//	caller spells the login URL and the poll endpoint from those and hands
//	them to the client; the login token opens in the system browser, where a
//	signed-in person approves with a POST. The client polls; while nobody has
//	approved the answer is pending, and once approved it carries the server,
//	the login name and an app password.
//
...
//  7. No URL spelling: the caller owns the wire vocabulary and this file
//     mints tokens and records who approved them.
package auth
...
// FlowTokens is what the begin answer hands a client: its own secret and the
// public one, so the caller can spell the URLs each belongs in.
...
// NewLoginFlowWithService builds a device login flow backed by auth's own
// credential mint. Keeping this adapter here avoids making the app wiring
// package define a credential policy or introducing a dependency cycle.
func NewLoginFlowWithService(st *state.DB, service *Service, now func() int64) (*LoginFlow, error) {
	if service == nil {
		return nil, errors.New("the login flow needs an auth service")
	}
	return NewLoginFlow(st, syncCredentialSource{service: service}, now)
}

type syncCredentialSource struct{ service *Service }

func (s syncCredentialSource) MintSyncCredential(ctx context.Context, user int64) (string, error) {
	token, _, err := s.service.CreateSyncCredential(ctx, user, "device login")
	return token, err
}
...
// Begin opens a flow and hands back the pair of tokens.
//
// The URL spellings a client uses to reach the login page and the poll
// endpoint are the caller's vocabulary, not this file's: it mints the two
// secrets and leaves the addressing to whoever owns the wire format.
