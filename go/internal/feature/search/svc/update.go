...
func (u *Updater) reconcile(ctx context.Context, ix *index.NameIndex, src search.Source, dir string) error {
	...
	ns := namespaceOf(uint32(src.NamespaceOf()))
	held, herr := ix.ChildrenOf(ns, dirPath)
	if herr != nil {
		return herr
	}
	onDisk := map[string]bool{}
	entries := make([]search.Entry, 0)
	rerr := src.ReaderOf().ReadDir(dirPath, func(e search.Entry) bool { entries = append(entries, e); return true })
	if rerr != nil && strings.Contains(strings.ToLower(rerr.Error()), "not found") {
		rerr = os.ErrNotExist
	}
	...
	if rerr == nil {
		for _, e := range entries {
			child := joinPath(dirPath, e.Name)
			if !src.Allows(child, e.Kind.IsDir()) {
				continue
			}
			onDisk[child] = true
		}
	...
	for _, p := range held {
		heldSet[p] = true
		if !onDisk[p] {
			removed = append(removed, index.Entry{Namespace: ns, Path: p})
		}
	}
	for p := range onDisk {
		if !heldSet[p] {
			added = append(added, index.Entry{Namespace: ns, Path: p})
		}
	}
...
func (u *Updater) sourceOf(share uint32) (search.Source, bool) {
	for _, src := range u.sources() {
		if uint32(src.NamespaceOf()) == share {
			return src, true
		}
	}
