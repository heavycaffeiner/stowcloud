...
func (s *Service) Build(
	...
	if !b.progress.Partial && !ix.CompleteCoverage(generation) {
		b.progress.Partial = true
	}
	return b.progress, nil
}
...
func (b *builder) walkSource(ctx context.Context, src search.Source) (bool, error) {
	...
	for len(stack) > 0 {
		...
		rerr := src.Reader.ReadDir(dir, func(e search.Entry) bool { entries = append(entries, e); return true })
		if rerr != nil {
			b.progress.Partial = true
			continue
		}
		...
		for _, e := range entries {
			...
			b.progress.Files++
			b.batch = append(b.batch, index.Entry{Namespace: index.Namespace(src.NamespaceOf()), Path: child})
