...
func (s *Service) Query(ctx context.Context, sources []search.Source, opt QueryOptions) (Results, error) {
	...
	if ix := s.index(); ix != nil && indexCanAnswer(opt) {
		it, plan, err := ix.Candidates(index.CandidateQuery{Needle: []byte(opt.Query)})
		if err == nil && it != nil {
			candidates := make([]index.Candidate, 0)
			for {
				candidate, nextErr := it.Next(ctx)
			...
			if closeErr := it.Close(); closeErr != nil {
				return Results{}, fmt.Errorf("search: closing index candidates: %w", closeErr)
			}
			if candidates != nil {
				hits := s.promote(ctx, sources, candidates, opt, needle)
				if ctx.Err() != nil {
					return Results{}, ErrCanceled
				}
				...
				return Results{Hits: hits, Tier: TierIndex, Truncated: truncated, Elapsed: s.clk.Now().Sub(start)}, nil
			}
		}
		if err == nil && plan.Fallback != index.FallbackNone {
			out, werr := s.walk(ctx, sources, needle, opt, start)
			out.Fallback = plan.Fallback
			return out, werr
		}
	}
...
func pathUnder(src search.Source, stored string) (string, error) {
	base := src.IndexBasePath()
	if base == "" {
		base = src.BasePath()
	}
	p := base
	for _, comp := range strings.Split(stored, "/") {
		...
		if comp == "." || comp == ".." || strings.IndexByte(comp, 0) >= 0 {
			return "", errors.New("search: invalid indexed path")
		}
		if p == "" {
			p = comp
		} else {
	...
	if base != "" && p != base && !strings.HasPrefix(p, base+"/") {
		return "", errors.New("search: an indexed path outside its source")
	}
	return p, nil
}
...
func (s *Service) promote(
	ctx context.Context, sources []search.Source, hits []index.Candidate, opt QueryOptions, needle []byte,
) []search.Hit {
	byNamespace := map[index.Namespace]search.Source{}
	for _, src := range sources {
		byNamespace[namespaceOf(uint32(src.NamespaceOf()))] = src
	}
	...
	for _, h := range hits {
		...
		src, ok := byNamespace[h.Namespace]
		if !ok || src.ReaderOf() == nil {
		...
		displayPath := search.JoinDisplay(src.Prefix, p)
		if opt.Scope != "" && !search.InScope(displayPath, opt.Scope) {
			continue
		}
		st, serr := src.ReaderOf().Stat(p)
		if serr != nil {
			continue
		}
		isDir := st.Kind.IsDir()
		if !src.Allows(p, isDir) {
			continue
		}
		...
		hit := search.Hit{
			Share: uint32(h.Namespace), Path: displayPath, Name: h.Name, IsDir: isDir,
			MTimeNs: mtime,
