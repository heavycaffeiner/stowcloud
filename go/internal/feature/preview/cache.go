...
// publicationUncertainError reports a cache entry whose rename happened but
// whose directory sync did not complete. The cache may contain either the old
// or new thumbnail after a restart, so callers retry rather than claiming that
// the old entry remains.
type publicationUncertainError struct{ err error }

func (e *publicationUncertainError) Error() string {
	return "preview: thumbnail publication uncertain: " + e.err.Error()
}

func (e *publicationUncertainError) Unwrap() error { return e.err }

// Put stores a thumbnail through a durable atomic replacement. If publication
// becomes uncertain, the returned error preserves that uncertainty so callers
// retry rather than claiming the prior cache entry remains.
...
func (c *Cache) Put(k Key, write func(*os.File) error) error {
	...
	res, err := fsatomic.ReplaceFileDurable(dest, 0o600, write)
	if err != nil {
		if res.Outcome == fsatomic.PublicationUncertain {
			return &publicationUncertainError{err: err}
		}
