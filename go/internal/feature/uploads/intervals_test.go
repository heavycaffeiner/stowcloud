...
func TestTheSetHasOneNormalFormWhateverTheOrder(t *testing.T) {
	t.Parallel()
	pieces := []Range{{Lo: 0, Hi: 10}, {Lo: 10, Hi: 20}, {Lo: 25, Hi: 30}, {Lo: 5, Hi: 15}, {Lo: 28, Hi: 40}}
	want := []Range{{Lo: 0, Hi: 20}, {Lo: 25, Hi: 40}}
...
func TestTouchingRangesMerge(t *testing.T) {
	t.Parallel()
	s := NewIntervalSet()
	for _, r := range []Range{{Lo: 0, Hi: 10}, {Lo: 10, Hi: 20}, {Lo: 20, Hi: 30}} {
		if err := s.Insert(r.Lo, r.Hi); err != nil {
...
func TestLoadRederivesTheInvariantAndRefusesCorruption(t *testing.T) {
	t.Parallel()
	s, err := LoadIntervalSet([]Range{{Lo: 20, Hi: 30}, {Lo: 0, Hi: 10}, {Lo: 5, Hi: 25}})
	if err != nil {
		t.Fatalf("LoadIntervalSet: %v", err)
	}
	if runs := s.Runs(); len(runs) != 1 || runs[0] != (Range{Lo: 0, Hi: 30}) {
...
func TestPrefixCompleteMissingAndReceived(t *testing.T) {
	t.Parallel()
	s := NewIntervalSet()
	for _, r := range []Range{{Lo: 10, Hi: 20}, {Lo: 30, Hi: 40}} {
		if err := s.Insert(r.Lo, r.Hi); err != nil {
			t.Fatalf("Insert: %v", err)
		}
	}
	...
	want := []Range{{Lo: 0, Hi: 10}, {Lo: 20, Hi: 30}}
