...
// Recovery keeps only recorded ranges backed by surviving physical chunks.
// One missing cache file must not erase a different chunk that remains.
func TestRecoveryIntersectsRecordedCoverageWithSurvivingChunks(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	spool := filepath.Join(t.TempDir(), "spool")
	f := newFixtureWithCache(t, spool)
	...
	const chunk = limits.UploadChunkFloor
	s := f.create(t, "partial-recovery.bin", uint64(chunk*4), SessionSpec{RandomAccess: true})
	f.patch(t, s.ID, uint64(chunk*2), chunkOf(uint64(chunk*2), chunk))
	f.patch(t, s.ID, uint64(chunk*3), chunkOf(uint64(chunk*3), chunk))

	dirs, err := os.ReadDir(spool)
	if err != nil || len(dirs) != 1 {
		t.Fatalf("reading session cache directory: entries=%d err=%v", len(dirs), err)
	}
	missing := filepath.Join(spool, dirs[0].Name(), cacheChunkName(uint64(chunk*3)))
	if removeErr := os.Remove(missing); removeErr != nil {
		t.Fatalf("removing one surviving chunk: %v", removeErr)
	}
	if recoverErr := f.engine.RecoverCache(ctx); recoverErr != nil {
		t.Fatalf("RecoverCache: %v", recoverErr)
	}
	after, err := f.engine.Get(ctx, s.ID, testUser)
	if err != nil {
		t.Fatalf("Get: %v", err)
	...
	if after.Received != chunk {
		t.Fatalf("recovery reported %d bytes, want the one surviving chunk %d", after.Received, chunk)
	}
}

// Close joins every background merger before returning, leaving no worker
// goroutine holding upload-owned resources.
func TestCloseJoinsBackgroundCacheMergers(t *testing.T) {
	t.Parallel()
	f := cached(t)
	s := f.create(t, "close-merger.bin", uint64(limits.UploadChunkFloor), SessionSpec{})
	m := f.engine.mergerFor(s.ID)
	...
	case <-m.done:
	default:
		t.Fatal("Close returned while the cache merger was still running")
	}
}
