...
// is reached through the AWS SDK for Go v2, which owns authentication,
// endpoint styles, XML protocol handling, and the bounded HTTP transport.
// A file whose bytes live in the bucket is materialized into server-owned
// scratch space before this package hands back a descriptor on it, exactly as
// vfs.Root documents: the handle type stays a real *vfs.File throughout the
// engine, and a backend that cannot offer one stages it.
package objstore
