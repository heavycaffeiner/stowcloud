...
// One login attempt needs four random values, and they are easy to confuse,
...
//     somebody else's browser, which is the login forgery the state alone does
//     not prevent.
...
// FlowSecrets are the secrets for one in-flight attempt.
//
// Three are stored as hashes. The verifier is kept as it is, because the
// exchange requires presenting the original.
...
// GoString redacts for the same reason, since a struct printed with the
// verbose verb bypasses String.
...
// one applies is decided per request by the layer that owns the host list. The
// same string has to reach the exchange byte for byte.
...
// scopes always contains the one that makes an identity token be issued at
...
func clientAuthMethod(doc *Discovery) (clientAuth, error) {
	...
	if slices.Contains(advertised, "client_secret_basic") {
		return authBasic, nil
	}
	if slices.Contains(advertised, "client_secret_post") {
		return authPost, nil
	}
...
// tokenResponse is what comes back. Only the identity token is read: an access
// token this server does not use is a credential it would then be holding.
...
//
// Every connection it makes passes through the address guard at dial time, so
// a hostname that resolved acceptably cannot be re-resolved to something else
...
func (c *Client) Exchange(ctx context.Context, code, redirectURI string, f FlowSecrets) (string, error) {
	...
	form := url.Values{}
	// Both of these are required by a conforming provider and both are easy to
	// leave out, because the exchange appears to work against the providers
	// that tolerate their absence.
	...
	if res.Error != "" {
		return "", &ProviderError{Reason: "the provider refused the exchange: " + res.Error}
	}
	if res.IDToken == "" {
