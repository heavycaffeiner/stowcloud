...
// An externally acquired instance lock is the host's responsibility. The
// engine can open under it without taking a duplicate lock, and closing the
// engine leaves the host lock held so an ordinary open is still refused.
func TestExternalInstanceOwnershipSkipsDuplicateLock(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	dir := t.TempDir()

	hostLock, err := instance.Take(dir)
	if err != nil {
		t.Fatalf("acquiring the host lock: %v", err)
	}
	t.Cleanup(func() {
		if releaseErr := hostLock.Release(); releaseErr != nil {
			t.Errorf("releasing the host lock: %v", releaseErr)
		}
	})
	...
	if err != nil {
		t.Fatalf("opening with external ownership: %v", err)
	}
	if err := e.Close(); err != nil {
