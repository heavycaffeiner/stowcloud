...
// there too.
func (e *Engine) filesArchive(c *gin.Context) {
	owner, ok := ownerOf(c)
	if !ok {
...
func (e *Engine) filesArchiveFetch(c *gin.Context) {
	owner, ok := ownerOf(c)
	if !ok {
		refuse(c, apierr.Classified{Class: apierr.AuthRequired})
		return
	...
	ticket, ok := e.Archives.Get(c.Query("token"), int64(owner), archive.KindArchive)
	if !ok {
	...
	for _, path := range ticket.Paths {
		resolved, err := e.resolve(owner, path, acl.Read|acl.Download)
		if err != nil {
			fail(c, err)
			return
		}
		encrypted, err := e.Core.ShareEncrypted(c.Request.Context(), resolved.Share())
		if err != nil {
			fail(c, err)
			return
		}
	...
	e.streamArchive(c, roots, ticket.Name)
}
...
func (e *Engine) streamArchive(c *gin.Context, roots []core.Resolved, name string) {
	...
	c.Header("Content-Disposition", httpheader.Attachment(name))
	c.Status(http.StatusOK)
	c.Writer.Flush()
	if err := e.buildArchive(context.WithoutCancel(c.Request.Context()), c.Writer, roots, name); err != nil {
...
func (e *Engine) filesArchiveList(c *gin.Context) {
	owner, ok := ownerOf(c)
	if !ok {
		refuse(c, apierr.Classified{Class: apierr.AuthRequired})
		return
	...
	resolved, err := e.resolve(owner, c.Query("path"), acl.Read|acl.Download)
	if err != nil {
		fail(c, err)
		return
	}
	...
	entry, random, err := e.Core.OpenRandom(c.Request.Context(), resolved)
	if err != nil {
		fail(c, err)
		return
	}
	defer func() {
		if closeErr := random.Close(); closeErr != nil {
			e.logger.Warn("closing an archive", "name", entry.Name, "error", closeErr)
		}
	}()

	listing, err := preview.ListArchive(c.Request.Context(), random, random.Size)
	if err != nil {
		notFound(c)
		return
	}
