...
func (e *Engine) linksList(c *gin.Context) {
	owner, ok := ownerOf(c)
	if !ok {
	...
	links, err := e.Core.ListLinks(c.Request.Context(), owner, at)
	if err != nil {
		fail(c, err)
		return
	}
	writeJSON(c, http.StatusOK, handler.LinksOf(links, e.linkVpath, e.now()))
}
...
func (e *Engine) adminLinksList(c *gin.Context) {
	if _, ok := e.admin(c); !ok {
		return
	}

	links, err := e.Core.ListAllLinks(c.Request.Context())
	if err != nil {
		fail(c, err)
		return
	}
	...
	if rows, uerr := e.Auth.ListUsers(c.Request.Context()); uerr == nil {
...
func (e *Engine) linksCreate(c *gin.Context) {
	owner, ok := ownerOf(c)
	if !ok {
		refuse(c, apierr.Classified{Class: apierr.AuthRequired})
	}
	...
	link, token, err := e.Core.CreateLink(c.Request.Context(), r, core.LinkSpec{
		Perms:    perms,
	...
	writeJSON(c, http.StatusCreated, view)
}
...
func (e *Engine) linksDelete(c *gin.Context) {
	owner, ok := ownerOf(c)
	if !ok {
		refuse(c, apierr.Classified{Class: apierr.AuthRequired})
	}
	...
	if err := e.Core.DeleteLink(c.Request.Context(), owner, id); err != nil {
		fail(c, err)
	}
...
func (e *Engine) linksUpdate(c *gin.Context) {
	owner, ok := ownerOf(c)
	if !ok {
		refuse(c, apierr.Classified{Class: apierr.AuthRequired})
	}
	...
	link, err := e.Core.UpdateLink(c.Request.Context(), owner, id, patch)
	if err != nil {
		fail(c, err)
	}
	writeJSON(c, http.StatusOK, handler.LinkOf(link, e.linkVpath(link), e.now()))
}
