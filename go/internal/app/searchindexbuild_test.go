...
func TestTheBuiltIndexSurvivesARestart(t *testing.T) {
	...
	// from nothing.
	second, oerr := app.Open(context.Background(), app.Options{DataDir: dataDir, PasswordParams: fastPasswordParams()})
	if oerr != nil {
		t.Fatalf("reopening: %v", oerr)
	}
	t.Cleanup(func() {
...
func TestTheLegacyIndexDirectoryMovesToTheHiddenName(t *testing.T) {
	...
	second, err := app.Open(ctx, app.Options{DataDir: dataDir, PasswordParams: fastPasswordParams()})
	if err != nil {
