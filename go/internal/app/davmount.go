...
func (e *Engine) mountDav(app *gin.Engine) {
	h := requestScoped(e.DavHandler(e.newDavHandler()))
...
// requestScoped replaces the framework's request context with one the handler owns.
// Database work derived from the request must not retain a framework-owned
// request object beyond the handler lifetime.
//
