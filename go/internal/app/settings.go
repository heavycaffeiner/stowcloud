...
func (e *Engine) applyThumbnailSettings(ctx context.Context, values runtimecfg.Values) {
	_ = ctx
	var old *preview.Service
	e.thumbnailMu.Lock()
	wasOn, wasDir, current := e.thumbnailOn, e.thumbnailDir, e.Preview
	if values.ThumbnailEnabled == wasOn && values.ThumbnailDir == wasDir && current != nil {
	...
	if !values.ThumbnailEnabled {
		...
		var next *preview.Service
		if e.Core != nil {
			next = openPreview(thumbsDir, e.previewWorker, e.Core, e.clock, e.logger)
		}
		old = current
		e.Preview = next
		e.thumbnailMu.Unlock()
