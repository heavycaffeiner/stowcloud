...
func (e *Engine) filesList(c *gin.Context) {
	owner, ok := ownerOf(c)
	if !ok {
	...
	raw := c.Query("path")
	if raw == "" || raw == "/" {
		writeJSON(c, http.StatusOK, rootPage(e.Core.Roots(owner)))
		return
	}
	...
	limit := 0
	if parsed, parseErr := strconv.Atoi(c.Query("limit")); parseErr == nil {
		limit = parsed
	}
	...
	page, err := e.Core.ListSorted(c.Request.Context(), r, core.Cursor(c.Query("cursor")), opt)
	if err != nil {
		fail(c, err)
		return
	}
	writeJSON(c, http.StatusOK, handler.PageOf(page, e.vpathOf(owner, r), e.refsOf(owner)))
}

// filesStat answers one entry.
func (e *Engine) filesStat(c *gin.Context) {
	owner, ok := ownerOf(c)
	if !ok {
		refuse(c, apierr.Classified{Class: apierr.AuthRequired})
		return
	...
	entry, err := e.Core.Stat(c.Request.Context(), r)
	if err != nil {
		fail(c, err)
		return
	}
