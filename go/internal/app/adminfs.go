...
func (e *Engine) adminFsBrowse(c *gin.Context) {
	...
	if err != nil {
		refuse(c, hostFsRefusal(err))
		return
	}
	writeJSON(c, http.StatusOK, listing)
}
...
// hold one.
func (e *Engine) setupFsBrowse(c *gin.Context) {
	if e.setup == nil {
		refuse(c, apierr.Classified{Class: apierr.SetupComplete, Key: "setup.complete"})
		return
	}
	...
	// consumes the token.
	if err := e.setup.Verify(c.Request.Context(), req.Token); err != nil {
