...
func apply(policy Policy, spec Spec, s steps) (Status, error) {
	...
	if s.reexeced(reexecMarker) {
		...
		st.Steps = append(st.Steps, StepStatus{Name: "landlock", Applied: true})
	} else {
		if ok, err := s.landlockAvailable(); !ok || err != nil {
			if err == nil {
				err = fmt.Errorf("this kernel reports no Landlock ABI")
			}
			st.Steps = append(st.Steps, StepStatus{Name: "landlock", Err: err})
			if policy == Required {
				return st, refusal(st)
			}
		} else {
			// The goroutine must not migrate between the restrict and the exec,
			// or the domain is left on a thread that is not the one that execs.
			runtime.LockOSThread()
			err := s.restrictAndReexec(spec, reexecMarker)
	...
	if err := s.installSeccomp(FilterProcess); err != nil {
		st.Steps = append(st.Steps, StepStatus{Name: "seccomp", Err: err})
		if policy == Required {
			return st, refusal(st)
		}
		return st, nil
	}
	st.Steps = append(st.Steps, StepStatus{Name: "seccomp", Applied: true})
	return st, nil
}
