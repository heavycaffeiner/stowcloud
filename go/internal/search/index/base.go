...
func WriteBase(entries []Entry, blockSize uint32, pruneDFRatio float32) ([]byte, error) {
	...
	for start, bid := 0, uint32(0); start < len(entries); start, bid = start+bs, bid+1 {
		...
		sortTrigrams(scratch)
		scratch = dedupTrigrams(scratch)
	...
	binary.LittleEndian.PutUint32(hdr[12:], uint32(blockCount)) //nolint:gosec // G115: bounded against MaxUint32 above.
...
func (s *BaseSegment) dictAt(i int) (t search.Trigram, flags byte, off, length uint32) {
	...
	return search.Trigram{e[0], e[1], e[2]}, e[3],
...
func sortTrigrams(v []search.Trigram) {
	sort.Slice(v, func(i, j int) bool { return bytes.Compare(v[i][:], v[j][:]) < 0 })
}

func dedupTrigrams(v []search.Trigram) []search.Trigram {
	if len(v) < 2 {
		return v
	}
	n := 1
