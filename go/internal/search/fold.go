...
func Fold(b []byte) []byte {
	if !utf8.Valid(b) {
		// Not UTF-8, so no Unicode operation is meaningful. ASCII case still
		// is, and everything else survives byte for byte.
		return asciiLower(b)
	}
	if isASCII(b) {
		// The common case, and the one worth keeping off the Unicode tables.
		return asciiLower(b)
	}
	// The full Unicode lowercase mapping, not strings.ToLower's simple one.
	// They differ on the characters whose lowercase form is longer than one
	// rune: Turkish dotted capital I lowercases to "i" plus a combining dot,
	// and a name spelled either way has to fold to the same bytes or it is
	// findable by only one of its spellings.
