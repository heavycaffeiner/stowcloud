...
// from outside: a WebSocket subscription pins the directory it is watching and
// releases it on unsubscribe.
//
...
// registration fits under the cap. It does not mutate the registration set: the
// caller removes the kernel watch first and then calls markUnregistered, so a
// failed removal cannot leave the two disagreeing.
