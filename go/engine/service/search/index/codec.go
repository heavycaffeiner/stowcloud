...
//
// The index is off by default, an escalation, and a cache: delete the
// directory and search keeps working on the walk tier. No durable truth lives
// here; everything in it is rebuildable from the shares.
//
// The on-disk format is fixed. Every structural byte this package writes is
// compared against a committed fixture, so a change here is a change to a
// format two implementations have to agree on.
package index
