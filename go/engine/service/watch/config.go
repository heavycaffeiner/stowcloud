...
// is seen immediately, which no container can promise:
// fs.inotify.max_user_watches cannot be raised from inside one, and a watch is
// per directory, so a large tree cannot be fully watched.
//
// The watcher is a sensor, not a writer. Consumers receive events and act; it
// never touches the cache itself.
package watch
...
// An operator who set it believed they had whole-mount watching, had
// per-directory watching, and the difference was invisible outside one startup
// log line. The value is gone until something implements it, and putting it
// back is a one-line addition on the day that stops being a lie.
type Backend uint8
