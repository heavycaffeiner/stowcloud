...
import (
	...
	"github.com/heavycaffeiner/stowcloud/go/internal/bootstrap/preflight"
	"github.com/heavycaffeiner/stowcloud/go/internal/bootstrap/sandbox"
	"go.uber.org/fx"
)
...
func run(addr, dataDir string, plain bool) error {
	...
	spec := hanami.Spec[preflight.Config]{
		Name: "sc-engine",
		Load: func(ctx context.Context) (preflight.Config, error) {
			return preflight.Load(ctx, preflight.Options{
				Addr:              addr,
				DataDir:           dataDir,
				Plain:             plain,
				Logger:            logger,
