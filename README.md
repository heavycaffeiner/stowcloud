...
not the first page. Everything, Files and Folders sit beside the query, so
narrowing to one of them is a single click; the File type menu adds groups
such as Images or Documents, each one a chip you can drop again, and the box
beside it takes extensions you type. Names only. Searching inside file
contents is a separate feature you turn on.
